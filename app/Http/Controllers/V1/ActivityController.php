...
class ActivityController extends BaseController
{
    ...
    public function index(ActivityRequest $request)
    {
       ...
       foreach ($data as $item){
           $item->link = config('config.domain').'?activity_id='.getEncryptStr($item->id);
    ...
    /**
     * @api {post} /admin/v1/activity 添加大转盘活动(基础设置)
    ...
    public function store(ActivityRequest $request)
    {
      ...
      if($data['code'] == 0){
          return $this->success('添加成功！',$data['msg']);
      }else{
    ...
    /**
     * @api {put} /admin/v1/activity/:id 编辑大转盘活动(基础设置)
    ...
    /**
     * @api {delete} /admin/v1/activity/:id 删除大转盘
    ...
    /**
     * @api {get} /admin/v1/activity/publish/:id 发布活动
     * @apiDescription 发布活动
     * @apiGroup Activity
     * @apiPermission none
     * @apiParam  id   大转盘id
     ...
     * }
     */
    public function publish($id,UserRepository $userRepository)
    {
        $activity = $this->activityRepository->getById($id);

        $user = $userRepository->getByWhereFirst(['user_id'=>$activity->user_id]);
        ...
        if($user->appid){
            ...
            return $this->error('请先配置公众号APPID！');
        }
    }

    //活动攻略
    /**
     * @api {post} /api/v1/desc 活动攻略
     * @apiDescription 活动攻略
     * @apiGroup Activity
     * @apiPermission none
    ...
    {
        $activity = $this->activityRepository->getActivity(getDecryptStr($request['activity_id']));
